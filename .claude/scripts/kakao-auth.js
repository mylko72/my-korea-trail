        ...
        res.on('end', () => {
          try {
            ...
            if (res.statusCode === 200) {
              ...
              reject(new Error(`토큰 발급 실패 (${res.statusCode}): ${json.error_description || json.error}`));
            }
          } catch (err) {
  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (error) {
        ...
        res.end(`<html><body><h1>❌ 오류 발생</h1><p>${error}</p></body></html>`);
        server.close();
...
async function main() {
  ...
  if (!process.env.KAKAO_REST_API_KEY) {
    ...
    console.error('   https://developers.kakao.com 에서 REST API 키를 복사하여 설정하세요.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`   - Access Token: ${tokenResponse.access_token.slice(0, 20)}...`);
    console.log(`   - Refresh Token: ${tokenResponse.refresh_token.slice(0, 20)}...`);
