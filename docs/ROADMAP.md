...
Notion을 CMS로 활용하여 코리아 둘레길(동해안·남해안·서해안·DMZ·지리산) 코스 정보를
...
## 현재 구현 완료 상태 (2026-03-23 기준)

Phase 1과 Phase 2의 주요 항목이 이미 완료된 상태입니다.
...
- [ ] `.env.local` 환경 변수 파일 생성 및 `.gitignore` 등록
  ```
  NOTION_API_TOKEN=
  NOTION_DATABASE_ID=
  NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=
  ```
- [ ] `next.config.ts` 이미지 도메인 허용 설정 (Notion 이미지 URL 대응)
  ```ts
  // notion.so, amazonaws.com 등 Notion 이미지 호스트 추가
...
- [ ] `/[category]/[slug]` 코스 상세 동적 라우트 생성
- [ ] `not-found.tsx` 404 페이지 생성
- [ ] `error.tsx` 에러 바운더리 페이지 생성
- [ ] `loading.tsx` 로딩 상태 페이지 생성 (각 라우트별)

### 완료 기준

- [ ] `npm run dev` 실행 시 에러 없이 `localhost:3000` 접속 가능
- [ ] `npm run build` 빌드 성공 (TypeScript 타입 에러 없음)
- [ ] `.env.local`에 실제 Notion API 토큰 및 DB ID 입력 완료
- [ ] `next/image` 컴포넌트로 Notion 이미지 정상 렌더링 확인

### 예상 소요 시간

**0.5일** (환경 변수 및 미완료 라우트 설정 위주, 이미 초기화 완료 상태)
...
- [ ] 홈 페이지에 최신 게시글 미리보기 섹션 추가 (F001 연동 전 Mock 데이터)
- [ ] 카테고리 페이지에 `PostGrid` + `TrailCard` 컴포넌트 교체 적용
- [ ] 페이지네이션 또는 무한 스크롤 UI 추가 (`PaginatedResult` 타입 활용)
- [ ] 게시글 카드에 `next/image` 적용 (현재 `<img>` 태그 사용 중 → 마이그레이션 필요)

#### F003: 코스 상세 표시

선행 의존성: `/[category]/[slug]` 라우트 생성 (Phase 1)

- [ ] `/[category]/[slug]/page.tsx` 상세 페이지 컴포넌트 생성
...
- [ ] 상세 페이지 레이아웃 구성
  ...
  - 거리/소요시간 인포 카드
  - 본문 콘텐츠 렌더링 영역
- [ ] Notion 블록 렌더러 (`NotionBlockRenderer.tsx`) 개발
...
- [ ] 카테고리 페이지 내 상단 필터 탭 UI 추가 (`CategoryFilter` 컴포넌트 적용)
  - URL 쿼리 파라미터(`?category=east-coast`) 또는 라우트 파라미터 방식 결정
- [ ] 홈 페이지에서 카테고리 클릭 시 해당 카테고리 페이지로 이동 확인

#### F005: 날짜 필터링

선행 의존성: `DateFilter.tsx` 완성 (Phase 2), F004 완성

- [ ] 카테고리 페이지에 날짜 필터 UI 통합 (`DateFilter` 컴포넌트 적용)
- [ ] URL 쿼리 파라미터(`?from=2024-01&to=2024-12`) 기반 필터 상태 관리
  - `useSearchParams` 훅 활용 (클라이언트 컴포넌트)
  - 서버 사이드 필터링 vs. 클라이언트 사이드 필터링 방식 결정
- [ ] 필터 초기화 버튼
...
- [ ] NavBar에 검색 아이콘/버튼 추가 및 검색 모달 또는 인라인 검색 UX 결정
- [ ] 검색 결과 페이지 (`/search?q=검색어`) 또는 카테고리 페이지 내 필터링 방식 결정
- [ ] 검색 대상 필드: 코스명(title), 카테고리(category), 설명(description)
- [ ] 검색 결과 없음 UI (EmptyState 컴포넌트)
- [ ] 검색어 하이라이팅 처리

### 완료 기준

- [ ] Mock 데이터 기준으로 홈, 카테고리, 상세 3개 페이지 UI 완성
- [ ] F004 카테고리 필터 → URL 변경 → 목록 갱신 흐름 동작 확인
- [ ] F005 날짜 필터 적용 시 목록이 올바르게 필터링됨
- [ ] F006 검색어 입력 → 결과 표시 흐름 동작 확인
- [ ] F003 Notion 블록 렌더러가 주요 블록 타입 모두 정상 렌더링
- [ ] 모바일(375px), 태블릿(768px), 데스크톱(1280px) 3개 해상도 UI 검수

### 예상 소요 시간

**4일** (상세 페이지 신규 개발 2일 + 필터/검색 UI 2일)
...
선행 의존성: Phase 3 완료, `.env.local` 환경 변수 설정 완료
...
| Phase | 작업 내용 | 예상 기간 | 시작일 | 완료 예정일 |
|---|---|---|---|---|
| Phase 1 | 프로젝트 초기 설정 | 0.5일 | 2026-03-23 | 2026-03-23 |
| Phase 2 | 공통 모듈/컴포넌트 개발 | 2일 | 2026-03-23 | 2026-03-25 |
| Phase 3 | 핵심 기능 개발 | 4일 | 2026-03-25 | 2026-03-29 |
| Phase 4 | 추가 기능 개발 | 2일 | 2026-03-29 | 2026-03-31 |
| Phase 5 | API 연동 + Playwright 테스트 | 3일 | 2026-03-31 | 2026-04-03 |
| Phase 6 | 최적화 및 배포 | 1.5일 | 2026-04-03 | 2026-04-05 |
| **합계** | | **13일** | **2026-03-23** | **2026-04-05** |
